...
mcp = FastMCP("vfx-mcp", description="Video editing MCP server with ffmpeg-python")
...
async def trim_video(
    ...
    """
    if ctx:
        await ctx.info(f"Trimming video from {start_time}s" + 
...
async def resize_video(
    ...
    if ctx:
        await ctx.info(f"Resizing video...")
...
async def get_video_metadata(filename: str) -> str:
...
def main():
    ...
    if transport == "stdio":
        print("Starting VFX MCP server on stdio...", file=sys.stderr)
        mcp.run()
    elif transport == "sse":
        host = os.environ.get("MCP_HOST", "localhost")
        port = int(os.environ.get("MCP_PORT", "8000"))
        print(f"Starting VFX MCP server on {host}:{port} (SSE)...", file=sys.stderr)
