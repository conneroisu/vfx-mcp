...
async def handle_ffmpeg_error(
    e: ffmpeg.Error, ctx: Context | None = None
) -> None:
...
def get_video_metadata(
    ...
    try:
        ...
        raise RuntimeError(
            f"Error analyzing video: {e}"
        ) from e


def create_standard_output(
    stream, output_path: str, **kwargs
) -> Any:
    ...
    return ffmpeg.output(
        stream, output_path, **default_settings
    )
